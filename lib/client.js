...
const viewSite = (clientUID, siteUID, siteName) => {
    ...
    const rtCheck = getRealTimeStatus(clientUID, siteUID);
                ...
                .then((url) => {
                    switch (category) {
                        ...
                        case 'monitor':
                            ...
                            newCellStyling(table, rowCount, 3, 'class', 'col-3 text-center');
                            if(userType === 'user' || userType === 'admin'){
                                newRow.insertCell(6).innerHTML = `<button class=' btn btn-outline-danger btn-sm' onclick="deleteDoc('${clientUID}', '${siteUID}', '${category}', '${docType}', '${fileName}', '${docType}')">Delete</button>`;
                                newCellStyling(table, rowCount, 4, 'class', 'col-3 text-center');
                            }
